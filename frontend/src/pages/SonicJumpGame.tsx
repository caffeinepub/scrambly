...
export default function SonicJumpGame() {
  ...
  const [highScore, setHighScore] = useState(() => {
    return parseInt(localStorage.getItem('sonicJumpHighScore') || '0', 10);
  });
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Player (Sonic circle)
      ctx.beginPath();
      ctx.arc(gs.player.x + PLAYER_SIZE / 2, gs.player.y + PLAYER_SIZE / 2, PLAYER_SIZE / 2, 0, Math.PI * 2);
      ...
      // Eye
      ctx.beginPath();
      ctx.arc(gs.player.x + PLAYER_SIZE / 2 + 6, gs.player.y + PLAYER_SIZE / 2 - 4, 4, 0, Math.PI * 2);
      ctx.fillStyle = '#fff';
      ctx.fill();
      ctx.beginPath();
      ctx.arc(gs.player.x + PLAYER_SIZE / 2 + 7, gs.player.y + PLAYER_SIZE / 2 - 4, 2, 0, Math.PI * 2);
      ctx.fillStyle = '#000';
      ctx.fill();
    ...
    <div className="min-h-screen bg-sonic-dark flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-3xl font-bold text-sonic-yellow font-fredoka">Sonic Jump</h1>
        ...
        <div className="bg-sonic-blue/20 rounded-2xl p-4 border-2 border-sonic-yellow/30 flex flex-col items-center gap-4">
          ...
          {!started && !gameOver && (
            <div className="text-center">
              <p className="text-white font-nunito mb-3">Press <kbd className="bg-sonic-yellow text-sonic-dark px-2 py-1 rounded font-bold">Space</kbd> or tap the canvas to jump!</p>
          ...
          {gameOver && (
            <div className="text-center">
              ...
              <p className="text-white font-nunito mb-3">Score: <strong>{score}</strong> {score >= highScore && score > 0 ? '🏆 New High Score!' : ''}</p>
              <Button onClick={resetGame} className="bg-sonic-yellow text-sonic-dark font-bold font-fredoka text-lg px-8 hover:bg-yellow-400">
          ...
          {started && !gameOver && (
            <p className="text-white/60 font-nunito text-sm">Press <kbd className="bg-white/20 px-1 rounded">Space</kbd> / <kbd className="bg-white/20 px-1 rounded">↑</kbd> or tap to jump</p>
          )}
        </div>
      </div>
    </div>
