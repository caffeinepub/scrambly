...
import { ArrowLeft, Users, UserCircle, Search } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-fredoka text-foreground">Find Friends</h1>
      ...
      {profile && (
        <div className="sonic-card p-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-nunito font-700 text-foreground">{profile.name}</p>
              <p className="text-sm text-muted-foreground font-nunito">
                Age: {userAge !== null ? userAge : 'Unknown'} • Born: {userBirthYear}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-secondary/10 border border-secondary/30 rounded-2xl p-4">
        <p className="font-nunito text-sm text-secondary-foreground">
          <strong>🎯 Age Matching:</strong> Scrambly connects you with fans born in{' '}
          <strong>{userBirthYear || '...'}</strong> — the same age as you!
      ...
      {!isAdmin ? (
        <div className="sonic-card p-8 text-center">
          ...
          <h3 className="font-fredoka text-xl text-foreground mb-2">Coming Soon!</h3>
          <p className="text-muted-foreground font-nunito text-sm max-w-sm mx-auto">
            Age-matched friend discovery is being rolled out. Check back soon to connect with Sonic fans your age!
          </p>
          <div className="mt-4 flex flex-wrap gap-2 justify-center">
            {['Sonic Fan', 'Shadow Fan', 'Tails Fan', 'Knuckles Fan'].map((tag) => (
              <span key={tag} className="text-xs font-nunito font-700 bg-primary/10 text-primary px-3 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-4">
          {!searchEnabled ? (
            ...
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground font-nunito">
                Found <strong>{matchedUsers.length}</strong> fan{matchedUsers.length !== 1 ? 's' : ''} your age!
              </p>
              {matchedUsers.map((user, i) => (
                <div key={i} className="sonic-card p-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-nunito font-700 text-foreground">{user.name}</p>
                    <p className="text-xs text-muted-foreground font-nunito">
                      Born {Number(user.birthYear)} • Age {currentYear - Number(user.birthYear)}
                    </p>
                  </div>
            ...
            <div className="sonic-card p-8 text-center">
              <Users size={40} className="mx-auto mb-3 text-muted-foreground" />
              <h3 className="font-fredoka text-xl text-foreground mb-1">No matches yet</h3>
              <p className="text-muted-foreground font-nunito text-sm">
                No other fans born in {userBirthYear} have joined yet. Invite your friends!
              </p>
            </div>
          )}
        </div>
