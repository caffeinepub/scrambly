...
export default function WhackAChaoGame() {
  ...
  const [highScore, setHighScore] = useState(() => parseInt(localStorage.getItem('whackChaoHS') || '0', 10));
  ...
  const chaoColors = ['#7EC8E3', '#A8E6CF', '#FFD3B6', '#FFAAA5', '#FF8B94', '#A8D8EA', '#AA96DA', '#FCBAD3', '#FFFFD2'];
    ...
    <div className="min-h-screen bg-sonic-dark flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-3xl font-bold text-sonic-yellow font-fredoka">Whack-a-Chao!</h1>
        ...
        <div className="bg-sonic-blue/20 rounded-2xl p-6 border-2 border-sonic-yellow/30">
          <div className="flex justify-between items-center mb-6">
            <div className="text-center">
              <p className="text-white/60 font-nunito text-xs uppercase tracking-wide">Score</p>
              <p className="text-sonic-yellow font-fredoka text-3xl">{score}</p>
            </div>
            <div className="text-center">
              <p className="text-white/60 font-nunito text-xs uppercase tracking-wide">Time</p>
              <p className={`font-fredoka text-3xl ${timeLeft <= 5 ? 'text-red-400' : 'text-white'}`}>{timeLeft}s</p>
            </div>
          </div>
          ...
          {!running && !finished && (
            <Button onClick={startGame} className="w-full bg-sonic-yellow text-sonic-dark font-bold font-fredoka text-lg hover:bg-yellow-400">
              Start Game!
            </Button>
          )}
          ...
          {finished && (
            <div className="text-center">
              <p className="text-sonic-yellow font-fredoka text-2xl mb-1">Time's Up!</p>
              <p className="text-white font-nunito mb-4">You whacked <strong className="text-sonic-yellow">{score}</strong> Chao! {score >= highScore && score > 0 ? '🏆 New High Score!' : ''}</p>
              <Button onClick={startGame} className="w-full bg-sonic-yellow text-sonic-dark font-bold font-fredoka text-lg hover:bg-yellow-400">
