...
import { ArrowLeft, CheckCircle, Send, ShieldCheck, XCircle } from 'lucide-react';
...
export default function AppealPage() {
  ...
  const handleSubmit = () => {
    ...
    if (appealUsed) {
      setError('You have already used your one appeal. No further appeals are available.');
      return;
    }

    if (!isValid) {
      setError(`Your appeal must be between ${MIN_LENGTH} and ${MAX_LENGTH} characters.`);
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full sonic-card p-8 text-center space-y-6">
          ...
          <div>
            <h1 className="font-fredoka text-3xl text-destructive mb-2">Appeal Already Used</h1>
            <p className="font-nunito text-muted-foreground text-sm leading-relaxed">
              You have already submitted an appeal. Each account is only allowed one appeal.
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full sonic-card p-8 text-center space-y-6">
          ...
          <div>
            <h1 className="font-fredoka text-3xl text-primary mb-2">No Active Ban</h1>
            <p className="font-nunito text-muted-foreground text-sm leading-relaxed">
              You don't currently have an active ban to appeal. Head back to search!
            </p>
          </div>
          <Button
            onClick={handleBack}
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      {showConfirmation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-sm p-4">
          <div className="max-w-sm w-full sonic-card p-8 text-center space-y-6 border-2 border-sonic-yellow/40">
            ...
            <div>
              <h2 className="font-fredoka text-2xl text-foreground mb-3">Appeal Approved!</h2>
              <div className="bg-sonic-yellow/10 border border-sonic-yellow/30 rounded-xl p-4">
                <p className="font-fredoka text-xl text-sonic-yellow mb-2">❤️ +1 Life</p>
                <p className="font-nunito text-sm text-foreground leading-relaxed font-bold">
                  1+ life — do not do it again, search something that's safe and Sonic related
                </p>
              </div>
              <p className="font-nunito text-xs text-muted-foreground mt-3">
                Your search access has been restored with +2% life. This is your final warning — any further violations will result in a permanent ban with no appeal.
              </p>
            </div>
            <Button
      ...
      <div className="max-w-lg w-full space-y-6">
        ...
        <div className="sonic-card p-6 text-center border-2 border-sonic-blue/30">
          ...
          <h1 className="font-fredoka text-3xl text-foreground mb-1">Ban Appeal</h1>
          <p className="font-nunito text-muted-foreground text-sm">
            Submit your appeal below. You only get <strong>one chance</strong> — make it count!
          </p>
        </div>
        ...
        <div className="sonic-card p-6 space-y-4">
          ...
          {!isAllowedOrigin && (
            <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4">
              <p className="font-fredoka text-sm text-destructive mb-1">⚠️ Wrong Site</p>
          ...
          <div className="space-y-2">
            <label className="font-fredoka text-sm text-foreground block">
            ...
            <div className="flex justify-between items-center">
              <span className={`text-xs font-nunito ${charCount < MIN_LENGTH ? 'text-destructive' : 'text-muted-foreground'}`}>
                {charCount < MIN_LENGTH
                  ? `${MIN_LENGTH - charCount} more characters needed`
                  : `${charCount} / ${MAX_LENGTH}`}
              </span>
              <span className={`text-xs font-nunito ${charCount > MAX_LENGTH ? 'text-destructive' : 'text-muted-foreground'}`}>
                {charCount > MAX_LENGTH ? `${charCount - MAX_LENGTH} over limit` : ''}
              </span>
            </div>
          </div>
          ...
          <div className="bg-sonic-blue/5 border border-sonic-blue/20 rounded-xl p-3">
            <p className="font-nunito text-xs text-muted-foreground leading-relaxed">
              📋 <strong>Important:</strong> This is your one and only appeal. If approved, you will receive +2% life (1 life restore) and your search access will be restored. Any further violations will result in a permanent ban with no further appeals.
            </p>
          </div>
