...
interface Obstacle { x: number; y: number; speed: number; }
...
export default function RingCollectorGame() {
  ...
  const [highScore, setHighScore] = useState(() => parseInt(localStorage.getItem('ringCollectorHS') || '0', 10));
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Move player
      if (keysRef.current.has('ArrowLeft') || keysRef.current.has('KeyA')) gs.player.x = Math.max(16, gs.player.x - PLAYER_SPEED);
      if (keysRef.current.has('ArrowRight') || keysRef.current.has('KeyD')) gs.player.x = Math.min(CANVAS_W - 16, gs.player.x + PLAYER_SPEED);
      ...
      for (const r of gs.rings) {
        if (!r.collected && Math.hypot(r.x - gs.player.x, r.y - gs.player.y) < RING_RADIUS + 16) {
          r.collected = true;
          gs.score++;
          setScore(gs.score);
        }
    ...
    <div className="min-h-screen bg-sonic-dark flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-3xl font-bold text-sonic-yellow font-fredoka">Ring Collector</h1>
        ...
        <div className="bg-sonic-blue/20 rounded-2xl p-4 border-2 border-sonic-yellow/30 flex flex-col items-center gap-4">
          ...
          {!started && !gameOver && (
            <div className="text-center">
              <p className="text-white font-nunito mb-3">Use <kbd className="bg-sonic-yellow text-sonic-dark px-2 py-1 rounded font-bold">← →</kbd> to collect rings, avoid red Badniks!</p>
          ...
          {gameOver && (
            <div className="text-center">
              ...
              <p className="text-white font-nunito mb-3">Rings: <strong>{score}</strong> {score >= highScore && score > 0 ? '🏆 New High Score!' : ''}</p>
              <Button onClick={resetGame} className="bg-sonic-yellow text-sonic-dark font-bold font-fredoka text-lg px-8 hover:bg-yellow-400">
