...
export default function FriendListItem({ friendPrincipal }: FriendListItemProps) {
  ...
  const displayName = profile?.name || friendPrincipal.toString().slice(0, 12) + '...';
  const initials = displayName.slice(0, 2).toUpperCase();

  return (
    <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-primary/5 transition-colors">
      <div className="w-10 h-10 rounded-full bg-primary/15 flex items-center justify-center shrink-0">
        <span className="font-fredoka text-primary text-sm font-bold">{initials}</span>
      </div>
