...
export default function FriendsModePage() {
  ...
  useEffect(() => {
    ...
    if (prev !== undefined && prev !== status) {
      if (status === "approved") {
        toast.success(
          "🎉 Great news! You are eligible for Friends Mode. Welcome!",
          {
            duration: 6000,
          },
        );
  ...
  const handleSubmitBirthdate = async () => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success(
        "Request submitted! The owner will review it within 1–2 days.",
      );
    } catch (err: any) {
      toast.error(
    ...
    <div className="max-w-lg mx-auto space-y-6">
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-fredoka text-lg leading-tight">
            Chat with Friends
          </p>
        ...
        <ChevronRight
          size={18}
          className="text-primary-foreground/60 group-hover:translate-x-0.5 transition-transform shrink-0"
        />
      </button>
            ...
            <div className="sonic-card p-5 space-y-4">
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
                <p className="font-nunito text-sm text-foreground">
                  🌟 <strong>Friends Mode is active!</strong> You can now
                  connect with other Scrambly fans your age. More features
                  coming soon!
                </p>
              </div>
            </div>
      ...
      {(status === "pending" || submitted) &&
        ...
        status !== "denied" && (
          <div className="sonic-card p-6 space-y-4 text-center">
            ...
            <div>
              <h2 className="font-fredoka text-xl text-foreground">
                Request Under Review
              </h2>
              <p className="font-nunito text-sm text-muted-foreground mt-1">
                Your Friends Mode request has been submitted and is being
                reviewed by the owner.
              </p>
            </div>
            <div className="bg-secondary/10 border border-secondary/20 rounded-xl p-3">
              <p className="font-nunito text-sm text-muted-foreground">
                ⏱ You will be notified within <strong>1–2 days</strong> once a
                decision has been made. This page checks for updates
      ...
      {status === "denied" && (
        <div className="sonic-card p-6 space-y-4 text-center">
          ...
          <div>
            <h2 className="font-fredoka text-xl text-destructive">
          ...
          <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-3">
            <p className="font-nunito text-sm text-muted-foreground">
              If you believe this is a mistake, please reach out to the Scrambly
      ...
      {!status && !submitted && (
        <div className="sonic-card p-6 space-y-5">
          <div className="space-y-2">
            ...
            <p className="font-nunito text-sm text-muted-foreground">
              Enter your birthdate to request access. The owner will review your
              request and notify you within 1–2 days.
            </p>
          </div>

          <div className="bg-primary/5 border border-primary/20 rounded-xl p-3">
            <p className="font-nunito text-xs text-muted-foreground">
              🔒 <strong>Privacy:</strong> Your birthdate is only used to match
              you with friends your age. It is reviewed by the owner and stored
              securely.
            </p>
          </div>
      ...
      <div className="bg-muted/40 rounded-2xl p-4 text-center">
        <p className="font-nunito text-xs text-muted-foreground">
          Friends Mode connects you with Scrambly fans born the same year as
          you. All requests are reviewed by the owner to keep the community
          safe. 💙
        </p>
      </div>
