...
import { AMY_ABILITY_ALIASES, sonicAbilities } from "../data/sonicAbilities";
...
function matchesAmyAbility(query: string): boolean {
  ...
  return AMY_ABILITY_ALIASES.some(
...
function ReplacementVideoResults({
  videos,
  title,
}: ReplacementVideoResultsProps) {
  return (
    <div className="mt-6">
...
export default function SearchPage() {
  ...
  const { data: backendResults = [], isLoading: backendLoading } =
    useSearchSonicContent(submittedQuery);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="bg-card rounded-2xl p-8 max-w-sm w-full text-center border border-border shadow-xl">
          ...
          <p className="text-muted-foreground mb-4">
            You've reached your search time limit set by your parent. Please
            check with your parent to continue searching.
          </p>
          <div className="flex items-center justify-center gap-2 text-muted-foreground text-sm">
            <Clock className="w-4 h-4" />
            <span>
              Time used: {Math.floor(searchTimeUsed / 60)} min{" "}
              {searchTimeUsed % 60} sec
            </span>
          </div>
        </div>
      </div>
  ...
  const isSonicExeQuery =
    submittedQuery.toLowerCase().trim() === "sonic.exe vs metal sonic";
  ...
  const localMatches: SonicKnowledgeEntry[] = trimmed
    ? ALL_LOCAL_ENTRIES.filter((entry) => {
        const haystack =
          `${entry.name} ${entry.description} ${entry.highlights}`.toLowerCase();
        return haystack.includes(trimmed);
      })
    : [];

  const amyAbilityMatches: SonicKnowledgeEntry[] =
    trimmed && matchesAmyAbility(submittedQuery)
      ? sonicAbilities.filter(
          (a) => !localMatches.some((m) => m.name === a.name),
        )
      : [];

  const channelMatches: (YouTubeChannelEntry & { content_type: "channel" })[] =
    trimmed
      ? youtubeChannels
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">
            Search for Sonic characters, abilities, items, and more!
          </p>
        </div>
