...
export default function SonicDodgeGame() {
  ...
  const [highScore, setHighScore] = useState(() =>
    Number.parseInt(localStorage.getItem("sonicDodgeHS") || "0", 10),
  );
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: spawnObs is defined in render scope but stable
  useEffect(() => {
    if (!started) return;
    ...
    const loop = () => {
      ...
      if (keysRef.current.has("ArrowLeft") || keysRef.current.has("KeyA"))
        gs.player.x = Math.max(PLAYER_R, gs.player.x - spd);
      if (keysRef.current.has("ArrowRight") || keysRef.current.has("KeyD"))
        gs.player.x = Math.min(CANVAS_W - PLAYER_R, gs.player.x + spd);
      if (keysRef.current.has("ArrowUp") || keysRef.current.has("KeyW"))
        gs.player.y = Math.max(PLAYER_R, gs.player.y - spd);
      if (keysRef.current.has("ArrowDown") || keysRef.current.has("KeyS"))
        gs.player.y = Math.min(CANVAS_H - PLAYER_R, gs.player.y + spd);
        ...
        <div className="bg-sonic-blue/20 rounded-2xl p-4 border-2 border-sonic-yellow/30 flex flex-col items-center gap-4">
          ...
          {gameOver && (
            <div className="text-center">
              ...
              <p className="text-white font-nunito mb-3">
                Survived: <strong>{score}s</strong>{" "}
