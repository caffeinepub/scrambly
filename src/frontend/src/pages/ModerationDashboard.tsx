...
export default function ModerationDashboard() {
  ...
  const { data: friendsRequests, isLoading: friendsLoading } =
    useGetAllFriendsModeRequests();
  ...
  const [reviewingPrincipal, setReviewingPrincipal] = useState<string | null>(
    null,
  );
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <h2 className="text-3xl font-fredoka text-destructive">
          Access Denied
        </h2>
        <p className="text-muted-foreground font-nunito text-center max-w-sm">
          This area is for Scrambly moderators only. You don't have permission
          to access this page.
        </p>
      </div>
    );
  ...
  const handleIssueWarning = async (
    ...
    try {
      ...
      const newCount = await issueWarning.mutateAsync({
        target,
        reason: warningReason.trim(),
      });
      toast.success(
        `Warning issued! User now has ${Number(newCount)} warning(s).`,
      );
      ...
      toast.error(
        "Failed to issue warning. Check the principal ID and try again.",
      );
    }
  };
  ...
  const handleReviewFriendsRequest = async (
    principal: string,
    status: "approved" | "denied",
  ) => {
    setReviewingPrincipal(principal + status);
    try {
      const success = await reviewFriendsRequest.mutateAsync({
        principal,
        status,
      });
      if (success) {
  ...
  const pendingFriendsRequests =
    friendsRequests?.filter((r) => r.status === "pending") ?? [];
  const reviewedFriendsRequests =
    friendsRequests?.filter((r) => r.status !== "pending") ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge variant="destructive" className="ml-auto font-nunito font-700">
          Admin
        </Badge>
      </div>
        ...
        {friendsLoading ? (
          ...
          <div className="space-y-3">
            {pendingFriendsRequests.map((req) => {
              ...
              const isApproving =
                reviewingPrincipal === `${req.principal}approved`;
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button
                        size="sm"
                        onClick={() =>
                          handleReviewFriendsRequest(req.principal, "approved")
                        }
                      ...
                      <Button
                        ...
                        variant="destructive"
                        onClick={() =>
                          handleReviewFriendsRequest(req.principal, "denied")
                        }
                        disabled={isApproving || isDenying}
        ...
        {ideasLoading ? (
          ...
          <div className="space-y-3">
            {unreviewedIdeas.map((idea, i) => {
              ...
              const submittedDate = new Date(
                Number(idea.timestamp) / 1_000_000,
              );
              const isMarking = markingIdeaIndex === globalIndex;

              return (
                // biome-ignore lint/suspicious/noArrayIndexKey: idea list, no stable unique id
              ...
              <details className="sonic-card p-4">
                ...
                <div className="mt-3 space-y-2">
                  {reviewedIdeas.map((idea, i) => {
                    ...
                    return (
                      <div
                        // biome-ignore lint/suspicious/noArrayIndexKey: reviewed ideas list, no stable unique id
                        key={i}
                        className="py-2 border-b border-border last:border-0 space-y-1"
                      >
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-xs font-nunito font-700 text-muted-foreground">
                            {idea.author}
                          </span>
                          <span className="text-xs text-muted-foreground font-nunito">
                            {formatDistanceToNow(submittedDate, {
      ...
      <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-4">
        <p className="text-sm font-nunito text-muted-foreground">
          <strong className="text-destructive">⚠️ Warning System:</strong> Users
          receive warnings for violating community guidelines. After{" "}
          <strong>3 warnings</strong>, their account is automatically locked and
        ...
        {postsLoading ? (
          ...
          <div className="space-y-3">
            {posts.map((post, i) => {
              ...
              return (
                // biome-ignore lint/suspicious/noArrayIndexKey: posts list, no stable unique id available
