...
export default function SonicRunnerGame() {
  ...
  const [highScore, setHighScore] = useState(() =>
    Number.parseInt(localStorage.getItem("sr_highscore") || "0"),
  );
  ...
  const draw = useCallback(() => {
    ...
    // Rings
    // biome-ignore lint/complexity/noForEach: canvas drawing loop, for..of would require extra checks
    ...
    // Shoes (animated)
    const legOffset = runner.onGround
      ? Math.sin(frameCountRef.current * 0.3) * 5
      : 0;
    ctx.fillStyle = "#e63946";
    ctx.beginPath();
  ...
  const gameLoop = useCallback(() => {
    ...
    if (
      frameCountRef.current %
        Math.max(60, 120 - Math.floor(scoreRef.current / 200)) ===
      0
    ) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start justify-center">
        <div className="sonic-card p-4 flex flex-col items-center">
          ...
          {/* biome-ignore lint/a11y/useKeyWithClickEvents: canvas game area, keyboard handled via keydown listener */}
          <div
          ...
          >
            ...
            {gameOver && (
              <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center gap-3">
                <div className="text-3xl font-fredoka text-destructive">
                  Game Over!
                </div>
                <div className="text-white font-nunito text-center space-y-1">
                  ...
                  <p>
                    Rings: <strong className="text-secondary">{rings}</strong>
                  </p>
                  <p>
                    Best:{" "}
