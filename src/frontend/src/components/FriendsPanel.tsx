...
import { useGetFriendRequests, useGetFriends } from "../hooks/useQueries";
...
export default function FriendsPanel({ isOpen, onClose }: FriendsPanelProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: backdrop dismiss is supplementary, close button handles keyboard */}
      <div
            ...
            {(requestsLoading || pendingRequests.length > 0) && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-fredoka text-base text-foreground">
                    Friend Requests
                  </h3>
                  {!requestsLoading && pendingRequests.length > 0 && (
                ...
                {requestsLoading ? (
                  ...
                  <div className="space-y-2">
                    {pendingRequests.map((req, idx) => (
                      <FriendRequestItem
                        key={`${req.requesterId.toString()}-${idx}`}
                        request={req}
                      />
                    ))}
                  </div>
            ...
            <div className="space-y-2">
              ...
              {friendsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center space-y-3">
                  ...
                  <div>
                    ...
                    <p className="font-nunito text-sm text-muted-foreground mt-1">
                      You haven't added any friends yet. Start connecting with
                      other Scrambly fans!
                    </p>
                  </div>
                  <div className="bg-primary/5 border border-primary/15 rounded-xl px-4 py-3 max-w-xs">
                    <p className="font-nunito text-xs text-muted-foreground">
                      💙 Friends Mode lets you connect with Scrambly fans your
                      age safely.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  {(friends as Principal[])
                    .slice(0, FRIEND_LIMIT)
                    .map((friendPrincipal, idx) => (
                      <FriendListItem
