...
export default function AgeVerificationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await verifyAge.mutateAsync({
        name: name.trim(),
      ...
      if (result.__kind__ === "ok") {
        ...
        setDenied(true);
        setDeniedReason(
          `You must be at least 10 years old to use Scrambly. Come back when you're older!`,
        );
      } else if (result.__kind__ === "tooOld") {
        setDenied(true);
        setDeniedReason(
          `Scrambly is designed for users aged 10–18. You're too old for this platform!`,
        );
      } else if (result.__kind__ === "locked") {
        setDenied(true);
        setDeniedReason(
          "Your account has been locked. Please create a new account.",
        );
        ...
        <div className="bg-card rounded-3xl p-10 max-w-md w-full mx-4 shadow-2xl text-center">
          ...
          <div className="bg-muted rounded-2xl p-4 mb-6">
            <p className="text-sm font-nunito font-700 text-foreground">
              Scrambly is for users aged{" "}
              <span className="text-primary">10 to 18 years old</span>.
            </p>
          </div>
          <Button
            onClick={onLogout}
            variant="outline"
            className="w-full rounded-full font-nunito font-700"
          >
            Logout
          </Button>
        </div>
    ...
    <div className="min-h-screen flex items-center justify-center sonic-gradient speed-lines">
      <div className="bg-card rounded-3xl p-10 max-w-md w-full mx-4 shadow-2xl">
        <div className="text-center mb-6">
          ...
          />
          <h2 className="text-3xl font-fredoka text-primary">
            Age Verification
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label
              htmlFor="name"
              className="font-nunito font-700 text-foreground"
            >
              Your Name
            </Label>
            <Input
          ...
          >
            {verifyAge.isPending ? (
              <span className="flex items-center gap-2">
                <Zap size={16} className="animate-spin" /> Verifying...
              </span>
            ) : (
              <span className="flex items-center gap-2">
                <CheckCircle size={16} /> Enter Scrambly!
              </span>
            )}
          </Button>
        </form>
