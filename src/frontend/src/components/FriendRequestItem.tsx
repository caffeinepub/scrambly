...
export default function FriendRequestItem({ request }: FriendRequestItemProps) {
  ...
  const handleAccept = async () => {
    try {
      await respond.mutateAsync({
        requesterId: requesterPrincipal,
        accept: true,
      });
  ...
  const displayName =
    profile?.name || `${requesterPrincipal.toString().slice(0, 12)}...`;
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-secondary/10 border border-secondary/20">
      <div className="w-10 h-10 rounded-full bg-secondary/20 flex items-center justify-center shrink-0">
        <span className="font-fredoka text-secondary-foreground text-sm font-bold">
          {initials}
        </span>
      </div>
