...
export type SearchEntry =
  | SonicKnowledgeEntry
  | (YouTubeChannelEntry & { content_type: "channel" });
...
function isChannelEntry(
  entry: SearchEntry,
): entry is YouTubeChannelEntry & { content_type: "channel" } {
    ...
    return (
      <div className="text-center py-12 mt-6">
        ...
        <p className="text-muted-foreground font-nunito">
          Try searching for "Knuckles", "Sonic", "Shadow", "youtube", or "Chaos
          Emerald"!
        </p>
      </div>
    );
    ...
    <div className="mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {results.map((entry, i) => {
          ...
          return (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: search results list, no stable unique id
              key={i}
            ...
            >
              ...
              {entry.highlights && (
                <div className="bg-secondary/10 rounded-xl p-2 mb-3">
                  <p className="text-xs font-nunito font-700 text-secondary-foreground">
                    ✨ {entry.highlights}
                  </p>
                </div>
              )}
